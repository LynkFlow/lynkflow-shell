...
const lynkflowPackages = Object.keys({ ...dependencies, ...devDependencies })
  .filter((name) => name.startsWith("@lynkflow/"));
...
if (lynkflowPackages.length === 0) {
  ...
  run(
    `npm install ${lynkflowPackages.join(" ")} --no-save ${SCOPE_OVERRIDE}`,
  );
}
